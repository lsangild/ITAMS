...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{p{4.3cm} X X}
		...
		\textbf{Action} & \textbf{Expected outcome} & \textbf{Outcome} \\
		\midrule
		Power on \systemName. Wait \SI{5}{\minute}. & \numrange{1}{5} valid data points are shown on the server. & The server receives $\approx$ 5 valid values pr. minute, but stops receiving after \SI{2}{\minute}, due to an unknown interrupt error on the \SAMD.\\
