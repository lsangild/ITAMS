...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{p{4.3cm} X X}
		...
		\textbf{Action} & \textbf{Expected outcome} & \textbf{Outcome} \\
		\midrule
		Send test command, via a USB to UART module, to \MKR coded with an Arduino Sketch forwarding the input to the \SARA. & The server log shows "Hello World". & Success. For more information see transcript in appendix \fxnote{Reference}\\
		\bottomrule
	\end{tabularx}
...
The used test command is \fxnote{Lasse aesthetics!}
\begin{quote}
	AT+USOST=\textit{SOCK},"\textit{IP}",\textit{PORT},11@Hello World
\end{quote}
Where \textit{SOCK} is decided by the \SARA module, and \textit{IP} and \textit{PORT} are decided by the server.\newline \newline
